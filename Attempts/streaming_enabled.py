...
- Real-time streaming output (token-by-token like ChatGPT)
...
class StreamingCallbackHandler(BaseCallbackHandler):
    """Custom callback handler for token-by-token streaming output."""
    
    def __init__(self):
        self.tokens = []
        self.is_streaming = False
        self.should_stream = True  # Flag to control streaming
    ...
    def on_llm_new_token(self, token: str, **kwargs: Any) -> None:
        ...
        if self.is_streaming and self.should_stream:
    ...
    def enable_streaming(self):
        """Enable streaming for regular responses."""
        self.should_stream = True

# Create streaming callback instance
...
# Toggle this if you want to see per-node streaming updates (in addition to token streaming)
...
def call_model(state: MessagesState):
    ...
    # Check if this is likely a tool call scenario by examining recent messages
    tool_call_likely = False
    if len(messages) >= 2:
        # If the last message is from tools, we're in a tool response cycle
        last_msg = messages[-1]
        if hasattr(last_msg, 'name') and last_msg.name in [tool.__name__ for tool in all_tools]:
            tool_call_likely = True
    
    if tool_call_likely:
        # Disable streaming for tool responses
        streaming_callback.disable_streaming()
        non_streaming_llm = ChatOpenAI(model=model, api_key=OPENAI_API_KEY, temperature=0)
        non_streaming_model = non_streaming_llm.bind_tools(all_tools)
        response = non_streaming_model.invoke(messages)
    else:
        # Enable streaming for user queries
        streaming_callback.enable_streaming()
...
# ──────────────────────────────────────────────────────────────────────────────
# Chat (REPL) interface with enhanced streaming
# ──────────────────────────────────────────────────────────────────────────────
def repl():
    print("Notion Agent Chat with Real-time Streaming. Type your question. Commands: /reset, /exit")
    ...
    while True:
        ...
        try:
            if STREAM_UPDATES:
                ...
                # Standard execution with streaming handled by callback
                out = app.invoke({"messages": messages})
        except Exception as e:
            elapsed = timer() - start_time
            print(f"\nEncountered an error after {elapsed:.2f}s -> {e}")
        ...
        # Handle output display - check if we need to print non-streamed responses
        final_ai_msgs = [m for m in messages if isinstance(m, AIMessage)]
        if final_ai_msgs:
            last_ai_msg = final_ai_msgs[-1]
            # If streaming was disabled or didn't happen, print the response
            if not streaming_callback.is_streaming and (not streaming_callback.should_stream or not last_ai_msg.content):
        ...
        # Log to file
        try:
            final_content = final_ai_msgs[-1].content if final_ai_msgs else "No response"
            with open(f"test logs/{model} test log.txt", "a", encoding="utf-8") as f:
                f.write(f"User query: {user}\nModel: {model}\nResponse length: {len(final_content)}\nResponse time: {elapsed:.2f}s\n\n\n")
        except Exception as e:
