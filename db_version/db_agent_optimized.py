...
- "is there a project named X" → TOOL (needs search_records_by_name)
- "find user john" → TOOL (needs search_records_by_name)
- "show me project 5" → TOOL (needs read_record)
- "list all clients" → TOOL (needs list_records)
- "create a task" → TOOL (needs create_record)
- "update project status" → TOOL (needs update_record)
- "delete user 3" → TOOL (needs delete_record)
- "how many records" → TOOL (needs get_database_stats)
- "current time" → TOOL (needs get_current_datetime)

IMPORTANT INSTRUCTIONS:
- For basic queries such as for asking the name of the project, confirmation, etc give decision "DIRECT".
- Carefully consider the entire conversation history, not just the latest message.
- If there is any chance (even 30% or more) that a tool/database function might be needed to fulfill the user's request, respond with TOOL.
...
def stream_presentation_response(messages: List[Any]) -> Generator[str, None, None]:
    ...
    try:
        ...
        fallback_content = "I've completed your request. The results have been processed successfully."
...
def should_continue(state: MessagesState):
    ...
    # If no tool calls, go to presentation layer instead of ending directly
    return "presentation"

def needs_presentation(state: MessagesState):
    """Always route to presentation layer after tools"""
    logger.info("🔄 Tool execution completed - routing to presentation layer")
    return "presentation"
...
# After tools, always go to presentation for formatting
workflow.add_edge("tools", "presentation")
...
class DatabaseAgent:
    ...
    def stream_response(self, user_input: str) -> Generator[str, None, None]:
        ...
        try:
            ...
            if will_call_tools:
                logger.info("🌊 stream_response: Tools will be called, using tool workflow with presentation.")
                # --- TOOLS PATH: Use streaming workflow that includes presentation ---
                ...
                # Always stream presentation layer for tool results
                ...
                logger.info("🌊 stream_response: No tools will be called, but still using presentation layer.")
                # --- NO-TOOLS PATH: Still use presentation layer for consistent formatting ---
                
                # Add the provisional AI response to history first
                self.conversation_history.append(provisional_ai)
                
                # Now stream through presentation layer for consistent formatting
                accumulated_response = ""
                for chunk in stream_presentation_response(self.conversation_history):
                    accumulated_response += chunk
                    yield chunk
                    
                # Replace the provisional response with the formatted one
                self.conversation_history[-1] = AIMessage(content=accumulated_response)
    ...
    def _needs_presentation_streaming(self, messages: List[Any]) -> bool:
        """Always needs presentation streaming now - both for tool results and direct responses"""
        return True  # Always route to presentation for consistent formatting
...
workflow_streaming.add_node("presentation", presentation_node)
...
workflow_streaming.add_edge("tools", "presentation")
# Presentation always ends
workflow_streaming.add_edge("presentation", END)
